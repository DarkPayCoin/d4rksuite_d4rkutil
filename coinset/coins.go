...
func (cs *CoinSet) TotalValue() (value partutil.Amount) {
...
func satisfiesTargetValue(targetValue, minChange, totalValue partutil.Amount) bool {
...
type CoinSelector interface {
	CoinSelect(targetValue partutil.Amount, coins []Coin) (Coins, error)
}
...
type MinIndexCoinSelector struct {
	MaxInputs       int
	MinChangeAmount partutil.Amount
}

// CoinSelect will attempt to select coins using the algorithm described
// in the MinIndexCoinSelector struct.
func (s MinIndexCoinSelector) CoinSelect(targetValue partutil.Amount, coins []Coin) (Coins, error) {
	cs := NewCoinSet(nil)
...
func (s MinNumberCoinSelector) CoinSelect(targetValue partutil.Amount, coins []Coin) (Coins, error) {
...
func (s MaxValueAgeCoinSelector) CoinSelect(targetValue partutil.Amount, coins []Coin) (Coins, error) {
	sortedCoins := make([]Coin, 0, len(coins))
	sortedCoins = append(sortedCoins, coins...)
...
func (s MinPriorityCoinSelector) CoinSelect(targetValue partutil.Amount, coins []Coin) (Coins, error) {
...
// partutil.Tx, a specific outpoint index, and the number of confirmations
...
func (c *SimpleCoin) Value() partutil.Amount {
	return partutil.Amount(c.txOut().Value)
}
