...
func TestFilterInsertKey(t *testing.T) {
	...
	wif, err := partutil.DecodeWIF(secret)
	if err != nil {
	...
	f.Add(partutil.Hash160(wif.SerializePubKey()))
...
func TestFilterBloomMatch(t *testing.T) {
	...
	tx, err := partutil.NewTxFromBytes(strBytes)
	if err != nil {
	...
	spendingTx, err := partutil.NewTxFromBytes(spendingTxBytes)
	if err != nil {
		t.Errorf("TestFilterBloomMatch NewTxFromBytes failure: %v", err)
		return
	}
